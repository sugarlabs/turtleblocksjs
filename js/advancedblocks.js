...
var evalArgDict = {
    ...
    'weatherincityhigh': 'var block = blocks.blockList[blk]; var apiURL = "https://george-vustrey-weather.p.mashape.com/api.php?location="; var conns = block.connections; var city = parseArg(activity, turtle, conns[1]); var daysAhead = parseInt(parseArg(activity, turtle, conns[2])); if (daysAhead < -1 || daysAhead > 5) { errorMsg("Days ahead must be in the range of -1 to 5."); daysAhead = 0; } if (block.cacheCity === city && block.cache !== undefined) { var response = block.cache; } else { var request = new XMLHttpRequest(); request.open("GET", apiURL + city, false); request.setRequestHeader("X-Mashape-Authorization", MASHAPE_KEY); request.send(null); var response = request.responseText; block.cacheCity = city; block.cache = response; } var forecast = JSON.parse(response); block.value = parseInt(forecast[daysAhead + 1]["high_celsius"]);',
    'weatherincitylow': 'var block = blocks.blockList[blk]; var apiURL = "https://george-vustrey-weather.p.mashape.com/api.php?location="; var conns = block.connections; var city = parseArg(activity, turtle, conns[1]); var daysAhead = parseInt(parseArg(activity, turtle, conns[2])); if (daysAhead < -1 || daysAhead > 5) { errorMsg("Days ahead must be in the range of -1 to 5."); daysAhead = 0; } if (block.cacheCity === city && block.cache !== undefined) { var response = block.cache; } else { var request = new XMLHttpRequest(); request.open("GET", apiURL + city, false); request.setRequestHeader("X-Mashape-Authorization", MASHAPE_KEY); request.send(null); var response = request.responseText; block.cacheCity = city; block.cache = response; } var forecast = JSON.parse(response); block.value = parseInt(forecast[daysAhead + 1]["low_celsius"]);',
};
