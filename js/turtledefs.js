...
function getMainToolbarButtonNames(name) {
    return (['popdown-palette', 'run', 'step', 'stop-turtle', 'hard-stop-turtle', 'clear', 'palette', 'hide-blocks', 'collapse-blocks', 'go-home', 'help', 'sugarizer-stop'].indexOf(name) > -1);
};


function getAuxToolbarButtonNames(name) {
    return (['planet', 'planet-disabled', 'open', 'save', 'paste-disabled', 'Cartesian', 'polar', 'utility', 'new-project', 'restore-trash'].indexOf(name) > -1);
}
...
function createHelpContent() {
    HELPCONTENT = [
        ...
        [_('Stop'), _('Stop the current project.') + ' ' + _('You can also type Alt-S to stop.'), 'header-icons/stop-turtle-button.svg'],
